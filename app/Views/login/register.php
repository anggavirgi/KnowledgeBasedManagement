...
<body class="m-0 p-0 h-full text-sm">
  ...
  <div class="h-screen flex justify-end">
    ...
    <?php if (!isset($user)) : ?>
      ...
      <?php else : ?>
        <div class="absolute w-[80%] md:w-[56%] lg:w-[45%] xl:w-[38%] top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white h-auto py-6 px-10 rounded-3xl drop-shadow-lg">
          <div>Welcome, please fill form first.</div>
          <div class="text-[#18A8D8] text-[2.1rem] font-bold mt-2 mb-5">Finish Register</div>
        <?php endif; ?>
          ...
          <?php if (!isset($user)) : ?>
            ...
            <div class="flex justify-between">
              ...
              <?php else : ?>
                ...
                <?php else : ?>
                  <button class="mt-4 py-2 text-center text-white rounded-md bg-[#115da5] hover:bg-[#0c5091]">Next</button>
                <?php endif; ?>
