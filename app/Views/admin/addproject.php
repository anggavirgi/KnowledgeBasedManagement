...
<div class="border-2 border-gray-200 bg-white p-7 rounded shadow-md text-sm">
    ...
    <form action="<?php echo base_url(); ?>kb/administrator/project" method="post" class="ms-7 my-10">
        <?php echo csrf_field(); ?>
        <div class="mb-5">
            <label for="name_project" class="block mb-2 text-sm font-medium text-gray-800">Project name</label>
            <input type="text" id="name_project" name="name_project" class="bg-gray-50 border text-gray-800 text-sm rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500 w-2/4 p-4 <?php if (session('errors.name_project')) : ?>border-red-600<?php endif ?>" placeholder="Project name" value="<?= old('name_project'); ?>">
